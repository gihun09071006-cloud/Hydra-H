...
Implements the :class:`AIProvider` interface with **deterministic mock**
responses that match HYDRA's expected contracts. This placeholder never calls an
external API, opens a socket, reads an API key, or uses randomness — it exists so
Engines can depend on the provider abstraction before any real model is wired in.
"""
